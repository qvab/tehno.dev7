...
$("#fieldlist"+opt.identifier+" .pbreak:not(.pb0)").find(".field").addClass("ignorepb");};var fform=function(){};$.extend(fform.prototype,{title:"Untitled Form",description:"This is my form. Please fill it out. It's awesome!",formlayout:"top_aligned",formtemplate:"",evalequations:1,autocomplete:1,request_cost:"",show:function(){return'<div class="fform" id="field"><h2>'+this.title+'</h2><span>'+this.description+'</span></div>';},after_show:function(id){$('#cp_calculatedfieldsf_pform'+id).attr('data-evalequations',this.evalequations).attr('autocomplete',((this.autocomplete)?'on':'off'));$('#cp_calculatedfieldsf_pform'+id).find('input,select').blur(function(){try{$(this).valid();}catch(e){};});$('#cp_calculatedfieldsf_pform'+id+' [name="couponcode"]').on('keyup',{'form':this.request_cost+id},function(){var e=$(this),v=e.val();if(v.length)
{if(e.next('.apply-coupon').length==0)
e.after('<div class="apply-coupon"></div>');}
else
{e.next('.apply-coupon').remove();}
e.closest('form').find('.discount-container').hide();});$(document).on('click','#cp_calculatedfieldsf_pform'+id+' .apply-coupon',{'field':this.request_cost,'sequence':id},function(evt){var me=$(this),c=me.prev('[name="couponcode"]'),v,r,f=me.closest('form'),fid=f.find('[name="cp_calculatedfieldsf_id"]');if(c.length&&f.length)
{v=c.val();r=$.getJSON(document.location.href,{"formid":fid.val(),"formsequence":evt.data.sequence,"couponcode":v,"action":"checkcoupon","_wpnonce":f.find('[id="_wpnonce"]').val()},function(data)
{try{var sequence=evt.data.sequence,field=evt.data.field+sequence,item=$.fbuilder.forms[sequence].getItem(field),price=item.val(),discount,final_price,dc=f.find('.discount-container');if(dc.length==0)
