...
class no_external_links_parser extends no_external_links
{
    var $debug_log = array();
    ...
    function encode_link($url)
    {
        ...
        if ($this->options['base64']) {
            ...
            $sql = 'select id from ' . $wpdb->prefix . NEL_TABLE_MASK_LINKS.' where lower(url)=lower(%s) limit 1';
    ...
    //             echo '<div class="error">' . __('Failed to make masked link. MySQL link table does not exist. Trying to create table.', NEL_I18N) . '</div>';
    ...
    //             $res = $wpdb->query($sql2);
    //             if (!$res) {
    //                 echo '<div class="error">' . __('Failed to create table. Please, check mysql permissions.', NEL_I18N) . '</div>';
    //                 $this->debug_info(__('Failed SQL: ', NEL_I18N) . '<br>' . $sql2 . '<br>' . __('Error was:', NEL_I18N) . '<br>' . $wpdb->last_error);
    //             } else {
    //                 echo '<div class="updated">' . __('Table created.', NEL_I18N) . '</div>';
    //                 $wpdb->query($sql);
    //             }
    //         } elseif (is_null($result)) {
            $this->debug_info(__('Failed SQL: ', NEL_I18N) . '<br>' . $sql . '<br>' . __('Error was:', NEL_I18N) . '<br>' . $wpdb->last_error);
            //}
            if (!$result) {
                $sql = 'INSERT INTO ' . $wpdb->prefix . NEL_TABLE_MASK_LINKS.' VALUES("", %s, %s)';
                $res=$wpdb->query($wpdb->prepare($sql, $url, md5($url)));
                if($res===0)
                {
                    $this->debug_info(__('Failed SQL: ', NEL_I18N) . '<br>' . $sql . '<br>' . __('Error was:', NEL_I18N) . '<br>' . $wpdb->last_error);
                }
    ...
    function decode_link($url)
    {
        ...
        if ($this->options['base64']) {
            ...
            $sql = 'select url from ' . $wpdb->prefix . NEL_TABLE_MASK_LINKS.' where id= %s limit 1';
    ...
    function check_redirect()#checking if it is redirect page
    {
        ...
        if (!empty($this->options['LINK_SEP']) && !empty($_REQUEST[$this->options['LINK_SEP']])) {
    ...
    function show_referer_warning()
    {
        ?>
        <html>
    <head><title><?php _e('Redirecting...', NEL_I18N); ?></title>
    ...
    <div align="center" style="margin-top: 15em;">
        <?php
        echo __('You have been redirected through this website from a suspicious source. We prevented it and you are going to be redirected to our ', NEL_I18N) . '<a href="' . get_home_url() . '">' . __('safe web site.', NEL_I18N) . '</a>'; ?>
    </div>
    </body></html><?php die();
    }
    ...
    function add_stats_record($url)
    {
        ...
        if (!$this->options['stats'])
            return;
        $sql = 'INSERT INTO ' . $wpdb->prefix . NEL_TABLE_LINKS_STATS.' VALUES("", %s ,NOW())';
        $res = $wpdb->query($wpdb->prepare($sql, $url));
        if ($res !== FALSE)
            return;#all ok
        #error - stats record could not be created
        $this->debug_info(__('Failed SQL: ', NEL_I18N) . '<br>' . $sql . '<br>' . __('Error was:', NEL_I18N) . '<br>' . $wpdb->last_error);
        // echo '<div class="error">' . __('Failed to save statistic data. Trying to create table.', NEL_I18N) . '</div>';
        ...
        // if ($res === FALSE) {
        //     echo '<div class="error">' . __('Failed to create table. Please, check mysql permissions.', NEL_I18N) . '</div>';
        //     $this->debug_info(__('Failed SQL: ', NEL_I18N) . '<br>' . $sql2 . '<br>' . __('Error was:') . '<br>' . $wpdb->last_error);
        // } else {
        //     echo '<div class="updated">' . __('Table created.', NEL_I18N) . '</div>';
        //     $wpdb->query($sql);
        // }
    ...
    <div align="center" style="margin-top: 15em;">
        ...
        elseif ($url)
            echo __('You were going to the redirect link, but something did not work properly.<br>Please, click ', NEL_I18N) . '<a href="' . $url . '">' . __('HERE ', NEL_I18N) . '</a>' . __(' to go to ', NEL_I18N) . $url . __(' manually. ', NEL_I18N);
        else
            _e('Sorry, no url redirect specified. Can`t complete request.', NEL_I18N); ?>
    </div>
    </body></html><?php die();
    }
    ...
    function fullmask_begin()
    {
        ...
        if (!$a)
            echo '<div class="error">' . __('Can not get output buffer!') . __('WP No External Links Can`t use output buffer. Please, disable full masking and use other filters.', NEL_I18N) . '</div>';
        if ($this->options['debug'])
    ...
    function fullmask_end($text)
    {
        ...
        if (!$text)
            $r .= '<div class="error">' . __('Output buffer empty!') . __('WP No External Links Can`t use output buffer. Please, disable full masking and use other filters.', NEL_I18N, 1) . '</div>';
        else {
            $r .= $this->debug_info("Processing text (htmlspecialchars on it to stay like comment): \n" . htmlspecialchars($text), 1);
            if (is_object($post) && (get_post_meta($post->ID, 'no_external_links_mask_links', true) == 2))
                $r .= $text;
    ...
    function set_filters()
    {
        ...
        if ($this->options['fullmask']) {
            ...
            add_filter('no_external_links', array($this, 'filter'), 99);
        }
    }
}
