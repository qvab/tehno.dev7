...
function generate_do_post_meta_item( $item ) {
	if ( 'date' === $item ) {
		...
		if ( $date ) {
            ...
            echo apply_filters( 'generate_post_date_output', sprintf( '<span class="posted-on">%1$s</span> ', $time_string ), $time_string );
		}
	}
	...
	if ( 'author' === $item ) {
		...
		if ( $author ) {
            ...
            echo apply_filters( 'generate_post_author_output', sprintf( '<span class="byline">%1$s</span> ', esc_html( get_the_author() )) );
		}
	}
